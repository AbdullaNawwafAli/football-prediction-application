...
Deno.serve(async (_req) => {
  ...
  return new Response(
    JSON.stringify({ success: true, teams_group_updated: updated }),
    { headers: { "Content-Type": "application/json" } }
  );
});


async function assignTeamGroups(
  supabase: SupabaseClient,
  matches: any[]
): Promise<{ updated: number; error: any }> {
  const groupMatches = matches.filter(
    (m: any) => m.stage === "GROUP_STAGE" && m.group_name
  );
  ...
  if (teamIds.length === 0) return { updated: 0, error: null };

  // Fetch teams that don't have a group assigned yet
  const { data: teamsWithoutGroup, error: fetchError } = await supabase
    .from("teams")
    .select("id, name")        // ← also fetch name for alphabetical ordering
    .in("id", teamIds)
    .is("group_name", null);

  if (fetchError) return { updated: 0, error: fetchError };
  if (!teamsWithoutGroup || teamsWithoutGroup.length === 0)
    return { updated: 0, error: null };

  // 1. Update group_name on teams
  const groupUpdates = teamsWithoutGroup.map((t: { id: number; name: string }) => ({
    id: t.id,
    name: t.name,
    group_name: teamGroupMap.get(t.id)!,
  }));

  const teamUpdateResults = await Promise.all(
    groupUpdates.map(({ id, group_name }) =>
      supabase.from("teams").update({ group_name }).eq("id", id)
    )
  );
  ...
  for (const t of groupUpdates) {
    ...
    byGroup.get(t.group_name)!.push({ id: t.id, name: t.name });
  }
  ...
  const { error: standingsError } = await supabase
    ...
    .upsert(
      ...
      { onConflict: "team_id", ignoreDuplicates: true } // PK is team_id; don't overwrite live stats
    );

  if (standingsError) return { updated: 0, error: standingsError };

  return { updated: groupUpdates.length, error: null };
}