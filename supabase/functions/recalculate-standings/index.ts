...
Deno.serve(async (_req)=>{
  ...
  const { data: matches, error: fetchError } = await supabase.from("matches").select("*").eq("stage", "GROUP_STAGE");
  if (fetchError) {
    return new Response(JSON.stringify({
      success: false,
      error: fetchError
  ...
  if (standingsRows.length === 0) {
    return new Response(JSON.stringify({
      success: true,
      message: "No standings to update",
      standings_updated: 0
    }), {
      headers: {
        "Content-Type": "application/json"
...
function buildStandings(matches) {
  ...
  for (const [, teams] of byGroup){
    teams.sort((a, b)=>b.points - a.points || b.goal_difference - a.goal_difference || b.goals_for - a.goals_for || a.booking_points - b.booking_points);
    teams.forEach((t, idx)=>{
