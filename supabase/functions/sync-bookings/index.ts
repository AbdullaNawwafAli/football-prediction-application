...
const BOOKING_STATUSES = new Set(["FINISHED"]);
...
Deno.serve(async (_req) => {
  ...
  if (activeKnockout && activeKnockout.length > 0) {
    return new Response(
      JSON.stringify({ success: true, message: "Knockout stage has begun — sync disabled", skipped: true }),
  ...
  if (!matches || matches.length === 0) {
    return new Response(
      JSON.stringify({ success: true, message: "No matches need booking sync", updated: 0 }),
      { headers: { "Content-Type": "application/json" } }
    );
  }
  ...
  for (const match of matches) {
    ...
    if (!res.ok) {
      errors.push({ matchId: match.id, error: `API error: ${res.status}` });
      continue;
    }
    ...
    for (const booking of (m.bookings ?? []) as any[]) {
      let pts = 0;
      switch (booking.card) {
        case "YELLOW": pts = 1; break;
        case "YELLOW_RED": pts = 3; break;
        case "RED": pts = 4; break;
      }
    ...
    const { error: updateError } = await supabase
      .from("matches")
      .update({ home_booking_score, away_booking_score })
      .eq("id", match.id);
