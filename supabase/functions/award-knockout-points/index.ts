...
Deno.serve(async (_req) => {
  ...
  if (predError) {
    return new Response(
      JSON.stringify({ success: false, error: predError }),
  ...
  if (!predictions || predictions.length === 0) {
    return new Response(
      JSON.stringify({ success: true, message: "No knockout predictions found", users_updated: 0 }),
  ...
  const { data: allCols, error: allColsError } = await supabase
    .from("knockout_round_points")
    .select("*");
  ...
  const { data: roundPoints, error: roundError } = await supabase
    .from("knockout_round_points")
    .select("stage, points");
  ...
  if (roundError) {
    return new Response(
      JSON.stringify({ 
        ...
        details: roundError,
        debug: { countError: countError?.message, allColsError: allColsError?.message }
      }),
      { headers: { "Content-Type": "application/json" }, status: 500 }
    );
  }
  ...
  if (!roundPoints || roundPoints.length === 0) {
    ...
    return new Response(
      JSON.stringify({ 
        ...
        debug: { allRows: allCols }
      }),
      { headers: { "Content-Type": "application/json" }, status: 500 }
    );
  }

  const pointsForStage = new Map<string, number>(
    roundPoints.map((r) => [r.stage, r.points])
  );
  ...
  const userKnockoutPoints = new Map<string, number>();
  ...
  for (const pred of predictions) {
    ...
    userKnockoutPoints.set(
      pred.user_id,
      (userKnockoutPoints.get(pred.user_id) ?? 0) + points_awarded
    );
  }
  ...
  const { error: predUpsertError } = await supabase
    .from("knockout_predictions")
    .upsert(updatedPredictions, { onConflict: "id" });

  if (predUpsertError) {
    return new Response(
      JSON.stringify({ success: false, error: predUpsertError }),
      { headers: { "Content-Type": "application/json" }, status: 500 }
    );
  }
  ...
  const { error: scoresError } = await supabase
    .from("user_scores")
    .upsert(userScoreRows, { onConflict: "user_id" });

  if (scoresError) {
    return new Response(
      JSON.stringify({ success: false, error: scoresError }),
      { headers: { "Content-Type": "application/json" }, status: 500 }
    );
  }
