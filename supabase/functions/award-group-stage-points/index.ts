...
const BATCH_SIZE = 100; // Process updates in batches to avoid hitting limits
...
Deno.serve(async (_req) => {
  ...
  while (true) {
    ...
    if (picks.length < pageSize) break; // Last page
    page++;
  }
  ...
  const actualPosition = new Map(
    (standings ?? []).map((s) => [s.team_id, s.group_position])
  );
  ...
  for (const pick of allPicks) {
    ...
    if (is_correct) {
      predictionTotals.get(pick.prediction_id).group_points +=
        POINTS_FOR_POSITION[pick.predicted_position] ?? 1;
    }
  }

  // ── 4. Batch upsert is_correct for all picks ────────────────────────────────
  for (let i = 0; i < updatedPicks.length; i += BATCH_SIZE) {
    const batch = updatedPicks.slice(i, i + BATCH_SIZE);
    const pickUpdatePromises = batch.map(({ id, is_correct }) =>
      supabase
        .from("group_stage_prediction_picks")
        .update({ is_correct })
        .eq("id", id)
    ...
    if (picksUpsertError) {
      return new Response(JSON.stringify({
        ...
        message: `Error at batch ${Math.floor(i / BATCH_SIZE)}`
      }), {
        headers: { "Content-Type": "application/json" },
        status: 500
      });
  ...
  // ── 6. Recompute feature1_points per affected user ───────────────────────────
  const affectedUserIds = [...new Set(predictionRows.map((r) => r.user_id))];

  const { data: groupTotals, error: groupTotalsError } = await supabase
    .from("group_stage_predictions")
    .select("user_id, group_points")
  ...
  const { data: knockoutTotals, error: knockoutTotalsError } = await supabase
